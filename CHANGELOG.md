...
- rct-clr publishes to Clojars as `sg.flybot/rct-clr`, so a consumer depends on a version instead of pinning a git sha ([#23](https://github.com/flybot-sg/rct-clr/issues/23))
...
- A `^:rct/test` block runs every form. A throw used to leave the block and skip the forms after it; it now reports an error against its own line ([#27](https://github.com/flybot-sg/rct-clr/issues/27))
